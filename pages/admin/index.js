...
const AdminDashboard = () => {
  ...
  const saveData = async () => {
    if (process.env.NODE_ENV !== "development") {
      alert("Saving data is available in development mode only.");
      return;
    }
    ...
    try {
      ...
      setStatusMessage("Saved updates successfully.");
    } catch (error) {
      console.error(error);
      setStatusMessage("Unable to save updates. Please try again.");
    }
  };
        ...
        <div className={`${theme === "dark" ? "bg-transparent" : "bg-white"}`}>
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              {statusMessage && (
                <span className="text-sm opacity-75">{statusMessage}</span>
              )}
